...
mapSingle :: (a -> a) -> [a] -> [[a]]
funcAux f xs c 
    | (c <= len) = [ys ++ [f x] ++ yss] ++ (funcAux f xs (c+1))
    | otherwise = []
    where
        len = (length xs)
        (ys, x:yss) = splitAt (c-1) xs
        
mapSingle f xs = funcAux f xs 1
...
-}
partitions :: [a] -> [[[a]]]
partitions xs = map reverse (foldr (\x r -> concatMap (funcHelp x) r) [[]] xs)
funcHelp x list = case list of
                    [] -> [[[x]]]
                    y:list -> ((x:y):list) : (map (y:) (funcHelp x list))
...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set graph = (length (S.toList (S.map (\x -> S.difference (outNeighbors x graph) set) set)) == 1) && 
                     (length (S.toList (S.map (\x -> S.difference (inNeighbors x graph) set) set)) == 1)
{-
    *** TODO ***
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = (concat (map (S.toList) (S.toList partition))) == (S.toList (nodes graph))
{-
    *** TODO ***
