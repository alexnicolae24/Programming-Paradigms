...
removeNode node graph = (S.delete node (nodes graph),S.fromList (filter (\x -> (fst x) /= node && (snd x) /= node) (S.toList (edges graph))))
...
{-
    *** TODO ***
    ...
    pentru noile noduri.
(S.delete old (S.fromList ((S.toList (nodes graph)) ++ news)), 
S.fromList ((filter (\x -> (fst x) /= node && (snd x) /= node) (S.toList (edges graph))) ++ (map news (filter (\x -> (fst x) == node || (snd x) == node) (S.toList (edges graph))))
))
    Exemplu:
...
splitNode old news graph = (S.delete old (S.fromList ((S.toList (nodes graph)) ++ news)),
                            S.fromList (filter (\x -> (fst x) /= old && (snd x) /= old) (S.toList (edges graph)) ++ 
                            [(x,y) | x <- news,y <- (S.toList (outNeighbors old graph))] ++ 
                            [(x,y) | x <- (S.toList (inNeighbors old graph)),y <- news]))
{-
    *** TODO ***
...
mergeNodes :: Ord a
           ...
           -> StandardGraph a  -- graful existent
           -> StandardGraph a  -- graful obținut
mergeNodes prop node graph = (S.fromList (filter (not . prop) (S.toList (nodes graph)) ++ 
                              (if (filter (not . prop) (S.toList (nodes graph))) /= (S.toList (nodes graph)) then (node : []) else [])),
                              S.fromList (map (\pair -> 
                                  if (prop (fst pair)) == False then
                                         (if (prop (snd pair)) == False then pair
                                          else ((fst pair),node))
                                 else 
                                    (if (prop (snd pair)) == False then (node,(snd pair))
                                     else (node,node))) (S.toList (edges graph))))
