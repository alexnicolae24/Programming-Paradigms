...
(let tuple-gen((Q-tup '((3 4 5))) (trans (list T1 T2 T3))) ;am folosit un named let caruia i-am dat ca parametrii o coada si o lista de transformari
      (stream-cons (car Q-tup)  ;apoi se adauga primul element din coada in stream la fiecare iteratie
                   (tuple-gen
                         (cdr (append Q-tup (list (multiply (car trans) (car Q-tup)) (multiply (cadr trans) (car Q-tup)) (multiply (caddr trans) (car Q-tup)))));se apeleaza generatorul
                          trans))))                                                                ;adaugandu-se in coada urmatorii trei termeni formati prin aplicarea transformarilor
...
(define (pairs G H)
  (let pair-gen((Gcop G) (Hcop H) (c 1) (it 0))  ;am folosit un named let in care am ca parametrii a copie a lui G,o copie a lui H,un contor ce reprez. nr. de col. si un alt contor
          (if (= c it)  
              (pair-gen G (stream-rest Hcop) (add1 c) 0)  ;in cazul in care cei doi contori sunt egali,se apeleaza generatorul cu fluxul G,ce ram din copia lui H primul cont
                                                          ;creste si al doilea se ap. cu 0
              (stream-cons (cons (stream-first Gcop) (stream-first Hcop))(pair-gen (stream-rest Gcop) Hcop c (add1 it))))))  ;altfel,se pune in stream perechea formata din primele
                                                                                                      ;elemente din fiecare copie si se apeleaza generatorul
...
(define G
  (let even1 ((seed 1))
    (stream-cons seed (even1 (add1 (add1 seed))))))

;creearea fluxului H
(define H
  (let even2 ((seed 3))
    (stream-cons seed (even2 (add1 (add1 seed))))))

;definirea functiei ce verifica daca numerele din pereche sunt prime intre ele
(define prime-between?
  (lambda (p)
    (if (= 1 (gcd (car p) (cdr p)))
        #t
        #f)))

;cu ajutorul lui stream-filter se scot din stream perechile de numere ce nu sunt prime intre ele
(define gh-pairs-stream
  (stream-filter prime-between? (pairs G H)))
...
;aceasta functie are rolul de a implementa 2 relatii ce sunt asemanatoare,dar difera printr-un semn
(define common
  (lambda(g h op) (/ (op (expt h 2) (expt g 2)) 2)))

;functia ce transforma o pereche intr-un tuplu ppt
(define to-ppt 
  (lambda (p)
    (let ((g (car p)) (h (cdr p)))
      (list (* (car p) (cdr p)) (common g h -) (common g h +)))))

;Mam folosit stream-map pentru a aplica functia de transformare pe fiecare pereche din flux
(define ppt-stream-in-pair-order
  (stream-map to-ppt gh-pairs-stream))
